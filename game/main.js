  ...
  function render(now) {
    ...
    if (!PAUSE && GAME) {
      ...
      for (i in dynamicObjects) {
        ...
        // Speed up train
        if (
          dynamicObjects[i].type === "TRAIN" ||
          (dynamicObjects[i].type === "COIN" && dynamicObjects[i].train)
        ) {
          dynamicObjects[i].translation[2] += 2 * speed;
        }
        // Accelerate
        if (
          Math.floor(distance) % 12000 == 0 &&
          distance > 12000 &&
          speed < 0.45
        ) {
          speed += 0.01;
          ...
          switch (dynamicObjects[i].type) {
            ...
            case "COIN":
              if (dynamicObjects[i].jetpack || dynamicObjects[i].train) {
                ...
                dynamicObjects[i].translation[2] = getRandomInt(
                  actual_distance,
                  5400
                );
            ...
            case "MAGNET":
              ...
              dynamicObjects[i].translation[2] =
                -1.0 *
                getRandomInt(
                  Math.abs(last_powerup) + 250,
                  Math.abs(last_powerup) + 400
                );
            ...
            case "BANANA":
              ...
              dynamicObjects[i].translation[2] =
                -1.0 *
                getRandomInt(
                  Math.abs(last_slip) + 100,
                  Math.abs(last_slip) + 200
                );
        ...
        // Handle collisions
        if (
          dynamicObjects[i].type === "COIN" ||
          dynamicObjects[i].type === "BOOT" ||
          dynamicObjects[i].type === "JETPACK" ||
          dynamicObjects[i].type === "MAGNET"
        ) {
          dynamicObjects[i].rotation[1] += 0.05;
          detectCollisionCollectible(i, gl);
        } else if (
          dynamicObjects[i].type === "BARRICADE" ||
          dynamicObjects[i].type === "TRAIN"
        ) {
        ...
        if (jetpack_flag && ground > GROUND_LEVEL - pushDown) {
          if (
            dynamicObjects[i].type == "COIN" &&
            dynamicObjects[i].jetpack == true
          ) {
          } else {
            dynamicObjects[i].translation[1] -= 0.05;
          ...
          dynamicObjects[i].translation[1] += 0.1;
          if (dynamicObjects[i].type === "GROUND" && first_ground) {
            ground += 0.1;
            first_ground = false;
          } else if (dynamicObjects[i].type === "GROUND" && !first_ground) {
        ...
        if (magnet_flag) {
          if (dynamicObjects[i].type == "COIN") {
            let x_distance = Math.abs(
              staticObjects[0].translation[0] - dynamicObjects[i].translation[0]
            );
            let z_distance = Math.abs(
              staticObjects[0].translation[2] - dynamicObjects[i].translation[2]
            );
        ...
        // Oil and Banana
        if (
          dynamicObjects[i].type === "OIL" ||
          dynamicObjects[i].type === "BANANA"
        ) {
          if (
            dynamicObjects[i].translation[0] ==
              staticObjects[0].translation[0] &&
            Math.abs(
              dynamicObjects[i].translation[2] - staticObjects[0].translation[2]
            ) < 15.0
          ) {
            let dist = Math.sqrt(
              Math.pow(
                ...
                Math.pow(
                  staticObjects[0].translation[1] -
                    dynamicObjects[i].translation[1],
                  2
                ) +
                Math.pow(
                  staticObjects[0].translation[2] -
                    dynamicObjects[i].translation[2],
                  2
                )
            );
            if (dist <= 0.65) {
              displayMessage(
                "YOU WERE SLOWED DOWN BY OIL OR BANANA PEEL. RUN FASTER!"
              );
              speed = MIN_SPEED;
              distance = 0.0;
              if (danger_flag) {
      ...
      for (i in staticObjects) {
        if (speed == 0 && staticObjects[i].type == "POLICE") {
          ...
          !danger_flag &&
          staticObjects[i].type == "POLICE" &&
          distance > 550.0 &&
          staticObjects[i].translation[2] < 3.0
        ) {
          staticObjects[i].translation[2] += 0.05;
        } else if (
          danger_flag &&
          staticObjects[i].type == "POLICE" &&
          staticObjects[i].translation[2] > -6.0
        ) {
